...
| R3 | "Preserve the raw encounter" — capture is a first-class entity | No capture entity existed; 324 legacy rows had no reconstructable per-row encounter | Capture is a first-class entity going forward. For the 324 existing rows, the raw per-row encounter is **not reconstructable**. Decision D4 (2026-09-12) records **ONE batch capture** for the 2026-09-11 rehabilitation import (`cap-2026-09-11-legacy-batch` + `legacy_batch_membership`), a true statement about the frozen archive + transform — not 324 fabricated encounters. See `D4_LEGACY_BATCH_CAPTURE.md`. |
...
| R5 | `canonical` must not imply `verified` | All but 4 rows are `unverified` yet are today's working corpus | The 324 rows are the current **canonical working set** (`corpus_state = canonical`, mostly `research_state = unverified`), gated by the Phase 0 retrofit acceptance + frontier review. This is the concrete proof that canonical ≠ verified. **`canonical` means "admitted to the Garden by a recorded gate", never "proven"** — it is a curation/admission property, not an evidence property. The counts behind this claim (324 rows; 320 `unverified`; 4 `verified` = ids 3, 12, 15, 26; 4 placeholder rows; 0 unresolved source links (closed 2026-09-13, decision D7 executed — see `docs/DECISIONS.md`); 27 tradition values; 32 source rows) are **re-derived from `quotes.csv`/`sources.csv` by `scripts/check_program_contracts.py`** rather than asserted in prose, so the doctrine cannot drift from the data silently. |
